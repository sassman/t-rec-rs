...
pub fn run_loop_for_seconds(seconds: f64) {
    unsafe {
        let future_date: *mut objc::runtime::Object =
            msg_send![class!(NSDate), dateWithTimeIntervalSinceNow: seconds];
        let runloop: *mut objc::runtime::Object = msg_send![class!(NSRunLoop), currentRunLoop];
...
mod tests {
    ...
    fn test_calculate_frame_top_right() {
        ...
        assert!((frame.origin.x - 900.0).abs() < 1.0);
        // y = 0/2 + 20 = 20
        assert!((frame.origin.y - 20.0).abs() < 1.0);
    }
    ...
    fn test_calculate_frame_center() {
        ...
        assert_eq!(frame.origin.x, 450.0);
        assert_eq!(frame.origin.y, 450.0);
    }
